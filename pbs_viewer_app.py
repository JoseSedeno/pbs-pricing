...
st.write(
    "MAX snapshot:",
    con.execute("SELECT MAX(snapshot_date) FROM fact_monthly").fetchone()
)

st.write(
    "August wide rows:",
    con.execute(
        'SELECT COUNT(*) FROM wide_fixed WHERE "AEMP Aug 26" IS NOT NULL'
    ).fetchone()
)
...
if dataset == "Chemo EFC":
    ...
    # PBS AEMP also uses these tables for month-to-month checks
    d_cols  = table_columns("dim_product_line")
    fm_cols = table_columns("fact_monthly")

    item_code_expr   = pick_col(d_cols, "item_code_b", "item_code")
    form_expr        = pick_col(d_cols, "attr_c", "legal_instrument_form", "form", default="NULL")
    line_brand_expr  = pick_col(d_cols, "brand_name", "brand", default="NULL")
    line_formul_expr = pick_col(d_cols, "attr_f", "formulary", default="NULL")
    resp_expr = pick_col(
        d_cols,
        "responsible_person", "responsible_person_name",
        "sponsor", "sponsor_name", "name_b",
        default="NULL"
    )
    ...
    if pd.isna(first_month) or pd.isna(last_month):
        ...
        coverage_df = (
            filtered_df.assign(_month=_m)
                      ...
                      .groupby(["series_id", "display_name"], as_index=False)["_month"]
...
with tab_price:
    ...
    if filtered_df.empty:
        ...
        x_domain_min = pd.to_datetime(filtered_df["month"].min()).normalize()
        ...
        def make_bucket_layers(df, align_value):
            ...
            for (dx_val, dy_val), sub_df in df.groupby(["label_dx", "label_dy"], dropna=False):
                layers.append(
                    alt.Chart(sub_df)
                    .mark_text(
                        dx=int(dx_val),
        ...
        text_layers = []
        text_layers += make_bucket_layers(label_top_left_df, "left")
        text_layers += make_bucket_layers(label_top_center_df, "center")
        text_layers += make_bucket_layers(label_top_right_df, "right")
        text_layers += make_bucket_layers(label_bottom_left_df, "left")
